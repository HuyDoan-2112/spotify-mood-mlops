"""
Multiclass classification training script.
Run with: `python -m src.train_multiclass --data multiclass.csv --target target_multiclass`
"""
...
def train(
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=test_size or config.DEFAULT_MULTICLASS_CONFIG.test_size,
        random_state=config.DEFAULT_MULTICLASS_CONFIG.random_state, stratify=y,
    )
    ...
    model_path = model_out or config.MODELS_DIR / "multiclass_model.joblib"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", default="multiclass.csv", help="Processed CSV filename in data/processed/")
    parser.add_argument("--target", default=config.DEFAULT_MULTICLASS_CONFIG.target, help="Target column name.")
